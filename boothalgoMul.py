...
def add(
    x, y
):  # function to carry out binary addition and returns the result as a string
    ...
    for i in range(
        maxlen - 1, -1, -1
    ):  # reverse order range, decreasing by 1 in every iteration
        r = carry
        r += 1 if x[i] == "1" else 0
        r += 1 if y[i] == "1" else 0
...
def TwoComp(
    n,
):  # function that returns the 2s complement of the binary input, both input and output are strings
    li = list(n)
